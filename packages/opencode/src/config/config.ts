...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "marsbotcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "marsbotcode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          ...
          for (const file of yield* ConfigPaths.files("marsbotcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
            yield* merge(file, yield* loadFile(file, authEnv), "local")
          }
        }
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir.endsWith(".marsbotcode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["opencode.json", "opencode.jsonc", "marsbotcode.json", "marsbotcode.jsonc"]) {
